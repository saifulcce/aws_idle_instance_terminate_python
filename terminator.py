...
if len(cpu_usage_list) < 15:
    ...
    if all(i < 20.0 for i in cpu_usage_list[-15:]):
        ...
        # Kill monitor.py process for not to taking any other samples.
        ...
        with open("/var/opt/usage.log", "a") as file:
            file.write("CPU is IDLE for last 15 mins, Terminating Instance.....")

        sleep(60)

        #Terminating Instance by getting Instance ID.
        response = client.terminate_instances(
            InstanceIds =  gimme_instence_id()
        )

    else:
        print("CPU usage Normal(more than 20%) as Application is running")
